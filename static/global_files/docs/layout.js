...
const serverUserAgent = getUserAgentFromCookie();
const urlCurrentWindow = new URL(window.location.href);

// Finishing user agent setup
const userAgentData = urlCurrentWindow.searchParams.get('user-agent');
if(userAgentData){
    urlCurrentWindow.searchParams.delete('user-agent');
...
// },1000);
if(serverUserAgent != userAgent){
    urlCurrentWindow.searchParams.set('user-agent', userAgent);

    // Generating animations
    const loadingUI = document.createElement('div');
    loadingUI.innerHTML = `
        <div class="pageLoadingCover">
            <img src="./static/global_files/media/logo-web-white.png" alt="Carfiy">
        </div>
    `;
    document.body.appendChild(loadingUI);
    setTimeout(() => {
        window.location.replace(urlCurrentWindow.toString());
    }, 1000);
...
window.onresize=()=>{
    if(!newSizeRequested){
        ...
        //     document.cookie = "user-agent=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path="+SERVER_ROOT;
...
function getUserAgentFromCookie() {
